...
**夜晚行动顺序**：
1. 狼人睁眼，选择击杀目标
2. 女巫睁眼，选择是否使用解药或毒药（若狼人击杀了玩家且女巫持有解药，可以选择救或不救；若女巫持有毒药，可以选择毒杀任意一名玩家）
3. 预言家睁眼，选择查验玩家身份
...
### 3.1 核心游戏模拟功能  
- **角色分配机制**：随机分配角色给7名AI玩家，确保角色配置符合7人局标准
- **游戏流程控制**：完全自动按顺序执行夜晚和白天阶段，严格遵循夜晚行动顺序
- **AI行动决策系统**：根据角色策略自动做出相应的行动决策，包括狼人击杀目标选择、预言家查验目标选择、女巫用药决策、猎人开枪决策和白天投票决策
- **胜负判定**：根据游戏规则自动判断胜利阵营，在每轮游戏结束后立即判定
- **对局记录**：记录每局对局行动消息到文本文件中，包括夜晚行动细节、白天投票情况和最终结果
  
### 3.2 AI策略系统  
- **策略接口设计**：定义所有角色共同遵循的策略接口，包含各类决策方法
- **内置策略实现**：  
...
- **策略绑定机制**：为不同角色或玩家绑定不同策略，支持策略组合测试，可配置不同角色使用不同策略
...
### 4.1 核心类设计  
- **Game类**：游戏主控制类，负责游戏流程的控制和胜负判定
- **Player类**：玩家类，包含玩家基本信息、角色、状态和策略
- **Role类**：角色类，定义不同角色的技能和特性
- **Strategy接口**：策略接口，定义AI决策方法
- **Statistics类**：统计类，负责收集和分析游戏数据
- **Log类**：日志类，负责记录游戏过程和结果
...
- 策略接口应包含决策方法如：选择击杀目标、选择查验目标、选择用药、选择开枪、选择投票等
  
### 5.2 随机数生成  
- 使用Java的Random类或ThreadLocalRandom生成随机数
- 确保随机数的分布均匀，避免出现偏向性
- 在关键决策点使用随机数时，确保随机性和可重复性（可通过设置种子实现）
...
- 提供简洁明了的结果输出格式
- 支持输出详细日志用于调试和分析
- 文件保存位置：对局记录和统计报告将保存在项目根目录下的logs文件夹中
  
## 6. 配置参数说明  
- **模拟轮数**：指定要运行的游戏轮数
- **策略组合**：为不同角色配置不同的AI策略
- **随机种子**：可选，设置随机数生成种子，用于复现特定结果
- **日志级别**：控制日志详细程度
- **输出格式**：控制统计结果的输出格式
  
## 7. 代码结构  
- **src/main/java**: 主要源代码目录
  - **com/wolfgame/core**: 核心游戏逻辑
  - **com/wolfgame/strategy**: AI策略实现
  - **com/wolfgame/stat**: 统计分析模块
  - **com/wolfgame/util**: 工具类
  - **Main.java**: 程序入口
...
3. 文件保存：保存log【对局id】.txt和report.txt至项目根目录下的logs文件夹中
